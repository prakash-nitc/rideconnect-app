...
export default function FindDriver() {
  ...
  const handleSearch = (query: string) => {
    ...
    filterDrivers(query, vehicleFilter);
  };

  const handleVehicleFilter = (value: string) => {
    setVehicleFilter(value);
    filterDrivers(searchQuery, value);
  };

  const filterDrivers = (query: string, vehicle: string) => {
    ...
    if (vehicle !== 'all') {
      filtered = filtered.filter((driver) => driver.vehicleType === vehicle);
    }
